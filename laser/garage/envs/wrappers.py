...
class BAMDPWrapper(gym.Wrapper):
    def __init__(self,
                 env,
                 episodes_per_task,
        ...
        if self.add_done_info:
            if isinstance(self.observation_space, gymnasium_spaces.Box):
                # FIXME
                if len(self.observation_space.shape) > 1:
                    raise ValueError  # can't add additional info for obs of more than 1D
                self.observation_space = gymnasium_spaces.Box(low=np.array([*self.observation_space.low, 0]),
    ...
    def reset(self, task=None):
        ...
        if self.add_done_info:
            if isinstance(state, torch.Tensor):
                state = torch.cat((state, torch.tensor([0.0], device=self.env.device)))
            else:
    ...
    # Similar to reset_mdp() (i.e. reset env without changing task), but also reset the episode_count
    # Similar to reset(), but do not change the task
    ...
    def step(self, action):
        ...
        if len(step) == 4:
            state, reward, self.done_mdp, info = step
        else:
            state, reward, self.done_mdp, _, info = step

        info['done_mdp'] = self.done_mdp

        if self.add_done_info:
            if isinstance(state, torch.Tensor):
                state = torch.cat((state, self.done_mdp.float()))
            else:
        ...
        done_bamdp = False if not isinstance(state, torch.Tensor) else torch.tensor([False], device=self.env.device)
        if self.done_mdp:
            self.episode_count += 1
            if self.episode_count == self.episodes_per_task:
                done_bamdp = True if not isinstance(state, torch.Tensor) else torch.tensor([True], device=self.env.device)
